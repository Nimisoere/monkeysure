...
import { homeInsuanceForm, homeRadioButtons } from '../../components/PageHeader/form-config';
import TextInput from '../../components/Shared/FormControls/TextInput';

const InsuranceForm = ({ type }) => (
  <div className="insurance-form">
    <PageHeader leadtext="Phone Insurance" subtext="easy as you like" />
    <div className="container mx-auto py-8">
      <form>
        <p className="text-lg font-normal">{homeInsuanceForm.header}</p>
        <p className="text-sm font-light">{homeInsuanceForm.description}</p>
        <fieldset className="mt-6">
          <div className="flex flex-wrap justify-between">
            {
              homeInsuanceForm.fields.map((field, index) => (
                <TextInput
                  key={index}
          ...
          <div className="home_insurance-radios">
                    ...
                    <div className="flex">
                      {
                        radio.options.map((option, optionIndex) => (
                          <div className="radio-control">
                            ...
                            <label for={`q_${index + 1}_${optionIndex}`} key={index}>
                              {option.label}
                            </label>
                          </div>
                        ))
          ...
          <button className="btn bg-primary mt-4 proceed-btn">Proceed</button>
        </fieldset>
      </form>
    </div>
  </div>
