...
const ListingCardList = ({ listing, btnConfig, type }) => {
  return (
    <div className={`listing-card-list grid ${listing.selected && type === 'compare' ? 'selected' : ''}`}>
      <div className="card-section w-3/12">
        <img className="card-logo mb-4" src={listing.logo} alt={listing.company} />
        <p className="company-name">{listing.company}</p>
      </div>

      <div className="card-section w-6/12">
        <p>{listing.details}</p>
        <p>
      ...
      <div className="card-section w-3/12 price-section">
